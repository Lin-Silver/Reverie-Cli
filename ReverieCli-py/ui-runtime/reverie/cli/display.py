...
class DisplayComponents:
    ...
    def _format_tool_argument_summary(self, arguments: Optional[Dict[str, Any]], max_items: int = 4) -> str:
        ...
        for key, value in list(arguments.items())[:max_items]:
            ...
            if key_text.lower() in large_text_keys:
                if isinstance(value, str):
                    line_count = value.count("\n") + (1 if value else 0)
                    rendered_text = f"<{len(value):,} chars"
                    if line_count > 1:
                        rendered_text += f", {line_count:,} lines"
                    rendered_text += ">"
                elif isinstance(value, (dict, list)):
                    rendered_text = f"<{type(value).__name__}, {len(value):,} item(s)>"
                else:
                    rendered_text = f"<{type(value).__name__}>"
            else:
                rendered = value
            ...
            parts.append(f"{key_text}={rendered_text}")
    ...
    def _format_live_stream_preview(self, text: str, *, max_lines: int = 8, max_chars: int = 1200) -> str:
        """Return a bounded tail preview for live tool output."""
        ...
        if len(preview) > max_chars:
            preview = preview[-max_chars:].lstrip()

        hidden_lines = max(0, len(lines) - len(selected))
        hidden_chars = max(0, len(raw_text) - len(preview))
        if hidden_lines or hidden_chars:
            prefix = f"... showing latest {len(selected)} line(s)"
            if hidden_lines:
                prefix += f", {hidden_lines} earlier line(s) hidden"
            preview = f"{prefix}\n{preview}"
        return preview
    ...
    def build_live_tool_panel(self, active_tools: List[Dict[str, Any]]) -> Panel:
        ...
        for index, item in enumerate(tool_items[:2]):
            ...
            argument_summary = self._format_tool_argument_summary(arguments, max_items=3)
            if argument_summary:
                body_parts.append(Text(argument_summary, style=self.theme.TEXT_DIM))
            if stdout_text:
                body_parts.append(Text("STDOUT", style=f"bold {self.theme.MINT_SOFT}"))
                body_parts.append(
                    Text(
                        self._format_live_stream_preview(stdout_text),
                        style=self.theme.TEXT_SECONDARY,
                    )
                )
            if stderr_text:
                ...
                body_parts.append(
                    Text(
                        self._format_live_stream_preview(stderr_text, max_lines=6, max_chars=900),
