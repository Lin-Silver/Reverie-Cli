...
impl LspManager {
    ...
    pub async fn start_server(&mut self, name: &str) -> Result<(), String> {
        ...
        let root_uri = config.root_uri.clone().or_else(|| {
            self.workspace_root.clone().map(|r| format!("file://{}", r))
        });
        
        if let Some(root) = root_uri {
            // 启动 LSP 服务器
            client
                .initialize(&root, &config.command, &config.args.iter().map(|s| s.as_str()).collect::<Vec<_>>())
                .await
            ...
            if let Some(info) = self.servers.get_mut(name) {
                ...
                info.started_at = Some(std::time::SystemTime::now()
    ...
    pub async fn start_all(&mut self) -> Result<(), String> {
        for (name, info) in &self.servers {
            if info.config.auto_start && info.state == LspServerState::Stopped {
                self.start_server(name).await?;
            }
        }
        Ok(())
    ...
    /// 通知文件打开
    pub async fn did_open(&mut self, file_path: &str, language_id: &str, text: &str) -> Result<(), String> {
        ...
        client.did_open(&uri, language_id, version, text).map_err(|e| e.to_string())?;
        
        Ok(())
    }
    
    /// 通知文件更改
    pub async fn did_change(&mut self, file_path: &str, version: i32, changes: Vec<TextDocumentContentChangeEvent>) -> Result<(), String> {
        let server_name = match self.get_server_for_file(file_path) {
            Some(name) => name,
            None => return Ok(()),
        };
    ...
    pub async fn get_diagnostics(&mut self, file_path: &str) -> Result<Vec<Diagnostic>, String> {
        ...
        client.get_diagnostics(&uri).await.map_err(|e| e.to_string())
    }
