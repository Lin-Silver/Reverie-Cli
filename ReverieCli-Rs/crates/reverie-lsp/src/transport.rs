...
impl LspTransport {
    ...
    pub fn start(&mut self, command: &str, args: &[&str]) -> Result<(), LspTransportError> {
        ...
        // 获取 stdin 和 stdout
        let stdin = child.stdin.take().ok_or(LspTransportError::ProcessFailed("Failed to get stdin".to_string()))?;
        let stdout = child.stdout.take().ok_or(LspTransportError::ProcessFailed("Failed to get stdout".to_string()))?;
        
        self.process = Some(child);
        
        // 启动读取线程
        let rx_in = self.tx.clone();
        thread::spawn(move || {
            let reader = BufReader::new(stdout);
            for line in reader.lines() {
                match line {
                    Ok(line) => {
                        if let Err(_) = rx_in.send(line) {
                            break;
                        }
                    }
                    Err(_) => break,
        ...
        thread::spawn(move || {
            for message in rx_in {
                if let Err(_) = writeln!(stdin, "{}", message) {
                    break;
                }
                let _ = stdin.flush();
            }
    ...
    pub fn send_request(&self, request: Request) -> Result<Response, LspTransportError> {
        let json = serde_json::to_string(&request).map_err(|e| LspTransportError::ParseError(e.to_string()))?;
        self.tx.send(json).map_err(|_| LspTransportError::ConnectionClosed)?;
        
        // 等待响应（简化处理，实际应该使用异步）
        thread::sleep(Duration::from_millis(100));
        
        // 这里应该实现真正的请求-响应匹配
        unimplemented!("Request-response matching not implemented");
    }
    
    /// 发送通知
    pub fn send_notification(&self, notification: Notification) -> Result<(), LspTransportError> {
        let json = serde_json::to_string(&notification).map_err(|e| LspTransportError::ParseError(e.to_string()))?;
        self.tx.send(json).map_err(|_| LspTransportError::ConnectionClosed)?;
        Ok(())
    }
    ...
    pub fn receive_response(&self) -> Result<Response, LspTransportError> {
        let line = self.rx.recv().map_err(|_| LspTransportError::ConnectionClosed)?;
