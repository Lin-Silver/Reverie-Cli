...
HELP_TOPICS: Dict[str, Dict[str, object]] = {
    ...
    "plugins": {
        ...
        "detail": "This is the runtime-plugin surface for Reverie-Gamer. Reverie scans only the plugin directories that actually exist under `.reverie/plugins`, reads `plugin.json` when present, supports richer packaged-entry metadata for Python-to-EXE plugins, probes runnable entries with the fixed Reverie CLI handshake `<plugin-name>.exe -RC`, and shows whether each detected plugin is compliant. Plugins that answer with valid JSON can expose `rc_*` dynamic tools directly to the model, while `/plugins templates` surfaces the local source templates used to author those wrappers.",
        "overview": "status, rescan, path, inspect <plugin-id>, scaffold <plugin-id>, validate <plugin-id>, build <plugin-id>, templates, template inspect <template-id>",
        "subcommands": [
            ...
            {"usage": "/plugins path", "description": "Show the source root, plugin install root, bundled runtime catalog code path, and template root."},
            ...
            {"usage": "/plugins scaffold <plugin-id>", "description": "Create a new source plugin tree from a bundled template such as `runtime_python_exe`."},
        ...
        "examples": ["/plugins", "/plugins rescan", "/plugins path", "/plugins inspect godot", "/plugins scaffold terrain_runtime", "/plugins validate terrain_runtime", "/plugins build terrain_runtime install", "/plugins templates", "/plugins template inspect runtime_python_exe"],
    },
    "mcp": {
        "command": "/mcp",
        "section": "Tools & Context",
