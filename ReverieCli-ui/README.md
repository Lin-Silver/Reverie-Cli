...
The Tools page reads the live flattened catalog from the CLI core, including descriptions, categories, aliases, supported modes, required arguments, MCP tools, runtime-plugin tools, and progressively loaded RATS tools.

The RATS page discovers running Reverie Engine services from executable-local `ReverieLocal/RATS/Services` directories. Enabling a service persists only the Engine path and exact permission set under `<cli-exe-root>/.reverie/rats/settings.json`; the Python core exclusively owns control/session tokens and native RTP calls. The page shows the compact catalog and retrieves full definitions on demand. Third-party service download/install is not part of the current verified slice.
