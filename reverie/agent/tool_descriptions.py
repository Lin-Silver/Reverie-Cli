...
def get_blender_modeling_workbench_description() -> str:
    """Description for the built-in Blender modeling workbench tool."""
    return """
## Blender Modeling Workbench Tool (blender_modeling_workbench)

Use this tool when Reverie should author Blender assets directly instead of relying on an external MCP server or Skill.
...
- Treat `blender_modeling_workbench` as the preferred built-in path for Blender DCC control, auditable `.blend` source generation, `.glb` exports, and preview renders. Do not present it as a standalone final AAA character-art generator; for humanoids, require the continuous body-core report, one armature, face landmarks, material roles, UVs, and import-contract evidence before accepting the candidate.
...
def _get_compact_tool_surface(mode: str) -> str:
    ...
    if normalized == "reverie":
        ...
        lines.append("- `blender_modeling_workbench`: control Blender through auditable authoring scripts when the task is explicitly about Blender modeling assets, including production-character scaffold audits and black-box iteration plans.")
    ...
    if normalized == "reverie-gamer":
        lines.extend(
            [
                ...
                "- `blender_modeling_workbench`: control Blender with plans/scripts, run Blender in background mode, export `.blend`/`.glb`/`.gltf`, render previews, generate production-character scaffold audit and black-box iteration evidence, and sync the model registry.",
                "- `game_models` runtime plugin (`rc_game_models_*`): prepare plugin-local venvs, select model/profile pairs, and download HuggingFace model packages for local text-to-3D, image-to-3D, or motion/model assistance under `.reverie/plugins/game_models/`; use TRELLIS Text XLarge with `profile=low_vram` for 8GB VRAM attempts.",
