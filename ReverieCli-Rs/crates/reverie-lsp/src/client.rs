...
impl LspClient {
    ...
    /// 初始化 LSP 客户端
    pub async fn initialize(&mut self, root_uri: &str, command: &str, args: &[&str]) -> Result<(), LspClientError> {
        // 启动传输层
        ...
        let request = Request {
            ...
            params: Some(serde_json::to_value(&params).map_err(|e| LspClientError::ParseError(e.to_string()))?),
        };
    ...
    /// 打开文本文档
    pub fn did_open(&mut self, uri: &str, language_id: &str, version: i32, text: &str) -> Result<(), LspClientError> {
        if !self.initialized {
    ...
    /// 更改文本文档
    pub fn did_change(&mut self, uri: &str, version: i32, changes: Vec<TextDocumentContentChangeEvent>) -> Result<(), LspClientError> {
        if !self.initialized {
            return Err(LspClientError::NotInitialized);
        }
