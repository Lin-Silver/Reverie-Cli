...
def _geminicli_parts_from_content(content: Any) -> List[Dict[str, Any]]:
    """Translate OpenAI-style text/image content into Gemini content parts."""
    ...
    for part in content:
        ...
        if data:
            parts.append({"inlineData": {"mimeType": mime_type or "image/png", "data": data}})
    return parts
    ...
    for message in messages:
        ...
        if role in ("system", "developer"):
            if len(messages) > 1:
                ...
                system_instruction["parts"].extend(
                    part for part in _geminicli_parts_from_content(content) if "text" in part
                )
                continue
            role = "user"
        ...
        if role == "user":
            ...
            node["parts"].extend(_geminicli_parts_from_content(content))
            payload["request"]["contents"].append(node)
