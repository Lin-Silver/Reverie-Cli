...
class CommandHandler:
    ...
    def _load_tti_config(self):
        ...
        tti_cfg["web_default_model_id"] = str(tti_cfg.get("web_default_model_id", "") or "").strip()
        tti_cfg["web_default_model_display_name"] = str(tti_cfg.get("web_default_model_display_name", "") or "").strip()
    ...
    def cmd_tti(self, args: str) -> bool:
        ...
        - /tti source      -> switch between local and web
        ...
        if not raw:
            self.console.print(
                ...
                f"Usage: /tti models OR /tti source [local|web] OR /tti add OR /tti <your prompt>"
    ...
    def _cmd_tti_source(self, value: str) -> bool:
        """Switch text-to-image source between local and web."""
        loaded = self._load_tti_config()
        if not loaded:
            self.console.print(f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Config manager not available[/{self.theme.CORAL_SOFT}]")
            return True
        ...
        if raw_value and not candidate:
            self.console.print(
                f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Invalid TTI source: {escape(raw_value)}. Use local or web.[/{self.theme.CORAL_SOFT}]"
            )
            return True
        if not candidate:
            current = normalize_tti_source(tti_cfg.get("active_source", "local"))
            candidate = Prompt.ask("TTI source", default=current, choices=["local", "web"]).strip().lower()
    ...
    def _cmd_tti_models(self) -> bool:
        ...
        active_source = normalize_tti_source(tti_cfg.get("active_source", "local"))

        if active_source == "web":
            ...
            selected = resolve_web_image_model(web_cfg, tti_cfg)
            current_selected_id = str(selected.get("id", "")).strip().lower() if selected else ""

            if not catalog:
                self.console.print(
                    f"[{self.theme.AMBER_GLOW}]{self.deco.DOT_MEDIUM} No Web image models are currently available. Check /web status or /web settings.[/{self.theme.AMBER_GLOW}]"
                )
                return True
            ...
            for index, item in enumerate(catalog):
                models_data.append({
                    ...
                    "description": f"{item['id']} | {item.get('description', '')}",
                    "model": item,
                })
                if str(item.get("id", "")).strip().lower() == current_selected_id:
                    current_model_id = str(index)
            ...
            try:
                ...
                self.console.print(f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Invalid Web image-model selection.[/{self.theme.CORAL_SOFT}]")
                return True

            if selected_idx < 0 or selected_idx >= len(catalog):
                self.console.print(f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Invalid Web image-model index.[/{self.theme.CORAL_SOFT}]")
                return True

            selected_item = catalog[selected_idx]
            tti_cfg["web_default_model_id"] = selected_item["id"]
            tti_cfg["web_default_model_display_name"] = selected_item["display_name"]
            config.text_to_image = tti_cfg
            config_manager.save(config)

            self.console.print(
                f"[{self.theme.MINT_VIBRANT}]{self.deco.CHECK_FANCY} Default Web TTI model set to: {selected_item['display_name']}[/{self.theme.MINT_VIBRANT}]"
            )
            if self.app.get('reinit_agent'):
                self.app['reinit_agent']()
            return True
    ...
    def cmd_web(self, args: str) -> bool:
        """Manage the WebAI2API-backed Web source."""
        raw = args.strip()
        if not raw:
            return self._cmd_web_status()

        lowered = raw.lower()
        if lowered in ("status", "check"):
            return self._cmd_web_status()
        if lowered in ("activate", "use"):
            return self._cmd_web_activate()
        if lowered == "model":
            return self._cmd_web_model("")
        if lowered.startswith("model "):
            return self._cmd_web_model(raw[6:].strip())
        if lowered in ("image-model", "image", "image model"):
            return self._cmd_web_image_model("")
        if lowered.startswith("image-model "):
            return self._cmd_web_image_model(raw[12:].strip())
        if lowered == "start":
            return self._cmd_web_start()
        if lowered == "stop":
            return self._cmd_web_stop()
        if lowered == "endpoint":
            return self._cmd_web_endpoint("")
        if lowered.startswith("endpoint "):
            return self._cmd_web_endpoint(raw[9:].strip())
        if lowered == "port":
            return self._cmd_web_port("")
        if lowered.startswith("port "):
            return self._cmd_web_port(raw[5:].strip())
        if lowered in ("auth", "token"):
            return self._cmd_web_auth("")
        if lowered.startswith("auth "):
            return self._cmd_web_auth(raw[5:].strip())
        if lowered.startswith("token "):
            return self._cmd_web_auth(raw[6:].strip())
        if lowered in ("setting", "settings", "ui"):
            return self._cmd_web_setting_ui()

        self.console.print(
            f"[{self.theme.AMBER_GLOW}]{self.deco.DOT_MEDIUM} Usage: /web [status|activate|model|image-model|start|stop|endpoint|port|auth|settings][/{self.theme.AMBER_GLOW}]"
        )
        return True

    def _resolve_web_source_id_for_command(self, web_cfg: Dict[str, Any], source_query: str) -> Optional[str]:
        from ..web import get_web_model_catalog, resolve_web_selected_model
        ...
        available_sources = {
            ...
            for item in get_web_model_catalog(model_type="text", enabled_adapters=web_cfg.get("enabled_adapters"))
        }
        wanted = str(source_query or "").strip().lower()
        if wanted:
            resolved = aliases.get(wanted, wanted)
            return resolved if resolved in available_sources else None

        selected = resolve_web_selected_model(web_cfg, model_type="text")
        selected_source = str((selected or {}).get("source_id", "")).strip().lower()
        if selected_source in available_sources:
            return selected_source
        return "chatgpt" if "chatgpt" in available_sources else (sorted(available_sources)[0] if available_sources else None)

    def _cmd_web_status(self) -> bool:
        from ..web import (
            get_web_service_status,
            normalize_web_config,
            resolve_web_image_model,
            resolve_web_selected_model,
        )
        ...
        selected_chat = resolve_web_selected_model(web_cfg, model_type="text")
        selected_image = resolve_web_image_model(web_cfg, tti_cfg)
        status = get_web_service_status(web_cfg, timeout_seconds=2)
        auth_token = str(web_cfg.get("auth_token", "") or "").strip()
        masked_token = "(not set)"
        if auth_token:
            masked_token = f"{auth_token[:6]}...{auth_token[-4:]}" if len(auth_token) > 12 else auth_token

        service_label = "Running" if status.get("running") else "Stopped"
        service_style = self.theme.MINT_SOFT if status.get("running") else self.theme.CORAL_SOFT
        lines = [
            f"[{self.theme.BLUE_SOFT}]Active model source:[/{self.theme.BLUE_SOFT}] {self._format_model_source_label(str(getattr(config, 'active_model_source', 'standard')).lower())}",
            f"[{self.theme.BLUE_SOFT}]Service:[/{self.theme.BLUE_SOFT}] [{service_style}]{service_label}[/{service_style}]",
            f"[{self.theme.BLUE_SOFT}]API URL:[/{self.theme.BLUE_SOFT}] {escape(str(web_cfg.get('api_url', '')))}",
            f"[{self.theme.BLUE_SOFT}]Endpoint:[/{self.theme.BLUE_SOFT}] {escape(str(web_cfg.get('endpoint', '')))}",
            f"[{self.theme.BLUE_SOFT}]Port:[/{self.theme.BLUE_SOFT}] {web_cfg.get('port', 3000)}",
            f"[{self.theme.BLUE_SOFT}]Auth token:[/{self.theme.BLUE_SOFT}] {escape(masked_token)}",
            f"[{self.theme.BLUE_SOFT}]Auto start:[/{self.theme.BLUE_SOFT}] {'ON' if bool(web_cfg.get('auto_start', True)) else 'OFF'}",
            f"[{self.theme.BLUE_SOFT}]Headless:[/{self.theme.BLUE_SOFT}] {'ON' if bool(web_cfg.get('headless', False)) else 'OFF'}",
            f"[{self.theme.BLUE_SOFT}]Adapters:[/{self.theme.BLUE_SOFT}] {escape(', '.join(web_cfg.get('enabled_adapters', [])) or '(none)')}",
            f"[{self.theme.BLUE_SOFT}]Source root:[/{self.theme.BLUE_SOFT}] {escape(str(web_cfg.get('source_root', '') or '(not found)'))}",
            f"[{self.theme.BLUE_SOFT}]Reference config:[/{self.theme.BLUE_SOFT}] {escape(str(web_cfg.get('config_path', '') or '(not resolved)'))}",
        ]
        if selected_chat:
            lines.insert(1, f"[{self.theme.BLUE_SOFT}]Chat model:[/{self.theme.BLUE_SOFT}] {escape(selected_chat['display_name'])} ({escape(selected_chat['id'])})")
        else:
            lines.insert(1, f"[{self.theme.BLUE_SOFT}]Chat model:[/{self.theme.BLUE_SOFT}] (none)")
        if selected_image:
            lines.insert(2, f"[{self.theme.BLUE_SOFT}]Image model:[/{self.theme.BLUE_SOFT}] {escape(selected_image['display_name'])} ({escape(selected_image['id'])})")
        else:
            lines.insert(2, f"[{self.theme.BLUE_SOFT}]Image model:[/{self.theme.BLUE_SOFT}] (none)")
        if status.get("running"):
            lines.append(f"[{self.theme.BLUE_SOFT}]Discovered models:[/{self.theme.BLUE_SOFT}] {status.get('models_count', 0)}")
            if status.get("safe_mode"):
                lines.append(
                    f"[{self.theme.AMBER_GLOW}]Safe mode:[/{self.theme.AMBER_GLOW}] {escape(str(status.get('safe_mode_reason', '') or 'enabled'))}"
                )
        elif status.get("error"):
            lines.append(f"[{self.theme.AMBER_GLOW}]Probe detail:[/{self.theme.AMBER_GLOW}] {escape(str(status.get('error')))}")

        self.console.print()
        self.console.print(
            Panel(
                Text.from_markup("\n".join(lines)),
                title=f"[bold {self.theme.PINK_SOFT}]{self.deco.SPARKLE} Web Source {self.deco.SPARKLE}[/bold {self.theme.PINK_SOFT}]",
                border_style=self.theme.BORDER_PRIMARY,
                box=box.ROUNDED,
                padding=(1, 2),
            )
    ...
    def _cmd_web_activate(self) -> bool:
        from ..web import normalize_web_config, resolve_web_selected_model
        ...
        if not selected:
            self.console.print(
                f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Web text-model catalog is empty. Check the bundled WebAPI references first.[/{self.theme.CORAL_SOFT}]"
            )
            return True

        web_cfg["selected_model_id"] = selected["id"]
        web_cfg["selected_model_display_name"] = selected["display_name"]
        config.web = web_cfg
        ...
        if self.app.get('reinit_agent'):
            self.app['reinit_agent']()
        self.console.print(
            f"[{self.theme.MINT_VIBRANT}]{self.deco.CHECK_FANCY} Web source activated.[/{self.theme.MINT_VIBRANT}]"
        )
        return True

    def _cmd_web_model(self, model_query: str) -> bool:
        from ..web import get_web_model_catalog, normalize_web_config
        ...
        config = config_manager.load()
        web_cfg = normalize_web_config(getattr(config, "web", {}))
        catalog = get_web_model_catalog(model_type="text", enabled_adapters=web_cfg.get("enabled_adapters"))
        return self._select_external_provider_model(
            config_attr="web",
            normalize_config=normalize_web_config,
            catalog=catalog,
            provider_label="Web",
            active_source="web",
            model_query=model_query,
    ...
    def _cmd_web_image_model(self, model_query: str) -> bool:
        from ..config import default_text_to_image_config
        from ..web import get_web_model_catalog, normalize_web_config
        ...
        config = config_manager.load()
        web_cfg = normalize_web_config(getattr(config, "web", {}))
        tti_cfg = config.text_to_image if isinstance(config.text_to_image, dict) else default_text_to_image_config()
        catalog = get_web_model_catalog(model_type="image", enabled_adapters=web_cfg.get("enabled_adapters"))
        if not catalog:
            self.console.print(
                f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Web image-model catalog is empty.[/{self.theme.CORAL_SOFT}]"
            )
            return True

        selected_model = self._resolve_catalog_selection(
            catalog=catalog,
            current_selected_id=str(tti_cfg.get("web_default_model_id", "")),
            model_query=model_query,
            provider_label="Web image",
        )
        if not selected_model:
            return True

        tti_cfg["web_default_model_id"] = selected_model["id"]
        tti_cfg["web_default_model_display_name"] = selected_model["display_name"]
        config.text_to_image = tti_cfg
        config_manager.save(config)

        self.console.print()
        self.console.print(
            f"[{self.theme.MINT_VIBRANT}]{self.deco.CHECK_FANCY} Default Web image model set to: {selected_model['display_name']} ({selected_model['id']})[/{self.theme.MINT_VIBRANT}]"
        )
        if self.app.get('reinit_agent'):
            self.app['reinit_agent']()
        return True

    def _cmd_web_start(self) -> bool:
        from ..web import normalize_web_config, start_web_service
        ...
        config = config_manager.load()
        web_cfg = normalize_web_config(getattr(config, "web", {}))
        result = start_web_service(web_cfg)
        if result.get("success"):
            self.console.print(
                f"[{self.theme.MINT_VIBRANT}]{self.deco.CHECK_FANCY} WebAI2API is ready at {escape(str(result.get('base_url', web_cfg.get('api_url', ''))))}[/{self.theme.MINT_VIBRANT}]"
            )
            return True

        self.console.print(
            f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Failed to start WebAI2API: {escape(str(result.get('error', 'unknown error')))}[/{self.theme.CORAL_SOFT}]"
        )
        log_tail = str(result.get("log_tail", "") or "").strip()
        if log_tail:
            self.console.print(Panel(escape(log_tail[-3000:]), title="WebAI2API Log Tail", border_style=self.theme.BORDER_SUBTLE, box=box.ROUNDED))
        return True

    def _cmd_web_stop(self) -> bool:
        from ..web import normalize_web_config, stop_web_service
        ...
        config = config_manager.load()
        web_cfg = normalize_web_config(getattr(config, "web", {}))
        result = stop_web_service(web_cfg)
        if result.get("success"):
            self.console.print(
                f"[{self.theme.MINT_VIBRANT}]{self.deco.CHECK_FANCY} WebAI2API stop request completed.[/{self.theme.MINT_VIBRANT}]"
            )
            return True

        self.console.print(
            f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Failed to stop WebAI2API: {escape(str(result.get('error', 'unknown error')))}[/{self.theme.CORAL_SOFT}]"
        )
        return True

    def _cmd_web_endpoint(self, endpoint_value: str) -> bool:
        from ..web import normalize_web_config
    ...
    def _cmd_web_port(self, port_value: str) -> bool:
        from ..web import normalize_web_config
        ...
        if not raw:
            raw = Prompt.ask("Web service port", default=str(web_cfg.get("port", 3000))).strip()
        try:
            parsed = int(raw)
        except ValueError:
            self.console.print(
                f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Web service port must be an integer.[/{self.theme.CORAL_SOFT}]"
            )
            return True
        if parsed < 1 or parsed > 65535:
            self.console.print(
                f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Web service port must be between 1 and 65535.[/{self.theme.CORAL_SOFT}]"
            )
            return True

        web_cfg["port"] = parsed
        web_cfg["api_url"] = f"http://127.0.0.1:{parsed}"
        config.web = web_cfg
        config_manager.save(config)

        self.console.print(
            f"[{self.theme.MINT_VIBRANT}]{self.deco.CHECK_FANCY} Web service port set to {parsed}.[/{self.theme.MINT_VIBRANT}]"
        )
        return True

    def _cmd_web_auth(self, value: str) -> bool:
        from ..web import normalize_web_config
        ...
        if not token:
            token = Prompt.ask("Web auth token", password=True, default=str(web_cfg.get("auth_token", ""))).strip()
        if not token:
            self.console.print(
                f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Web auth token cannot be empty.[/{self.theme.CORAL_SOFT}]"
            )
            return True

        web_cfg["auth_token"] = token
        config.web = web_cfg
        config_manager.save(config)
        self.console.print(
            f"[{self.theme.MINT_VIBRANT}]{self.deco.CHECK_FANCY} Web auth token updated.[/{self.theme.MINT_VIBRANT}]"
        )
        return True

    def _cmd_web_login(self, source_query: str) -> bool:
        from ..web import normalize_web_config, run_web_login_session
        ...
        config = config_manager.load()
        web_cfg = normalize_web_config(getattr(config, "web", {}))
        source_id = self._resolve_web_source_id_for_command(web_cfg, source_query)
        if not source_id:
            self.console.print(
                f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Could not resolve a Web source to log in.[/{self.theme.CORAL_SOFT}]"
            )
            return True

        self.console.print(
            f"[{self.theme.TEXT_DIM}]Launching an interactive login session for {escape(source_id)}. Finish the browser-side flow, then return here and press Enter.[/{self.theme.TEXT_DIM}]"
        )
        result = run_web_login_session(web_cfg, source_id)
        if not result.get("success"):
            self.console.print(
                f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Web login session failed: {escape(str(result.get('error', 'unknown error')))}[/{self.theme.CORAL_SOFT}]"
            )
            return True

        auth_state = result.get("auth_state") if isinstance(result.get("auth_state"), dict) else {}
        lines = [
            f"[{self.theme.BLUE_SOFT}]Source:[/{self.theme.BLUE_SOFT}] {escape(source_id)}",
            f"[{self.theme.BLUE_SOFT}]Browser profile:[/{self.theme.BLUE_SOFT}] {escape(str(result.get('browser_profile', '') or '(auto)'))}",
            f"[{self.theme.BLUE_SOFT}]Browser account:[/{self.theme.BLUE_SOFT}] {escape(str(result.get('browser_account_email', '') or '(unknown)'))}",
            f"[{self.theme.BLUE_SOFT}]Runtime profile:[/{self.theme.BLUE_SOFT}] {escape(str(result.get('runtime_user_data_dir', '') or '(n/a)'))}",
        ]
        if auth_state:
            lines.append(f"[{self.theme.BLUE_SOFT}]Arena cookie present:[/{self.theme.BLUE_SOFT}] {'YES' if auth_state.get('arena_cookie_present') else 'NO'}")
            lines.append(f"[{self.theme.BLUE_SOFT}]Google session visible:[/{self.theme.BLUE_SOFT}] {'YES' if auth_state.get('google_web_signed_in') else 'NO'}")
            lines.append(f"[{self.theme.BLUE_SOFT}]Page login visible:[/{self.theme.BLUE_SOFT}] {'YES' if auth_state.get('page_login_visible') else 'NO'}")

        self.console.print()
        self.console.print(
            Panel(
                Text.from_markup("\n".join(lines)),
                title=f"[bold {self.theme.PINK_SOFT}]{self.deco.SPARKLE} Web Login {self.deco.SPARKLE}[/bold {self.theme.PINK_SOFT}]",
                border_style=self.theme.BORDER_PRIMARY,
                box=box.ROUNDED,
                padding=(1, 2),
            )
    ...
    def _cmd_web_auth_state(self, source_query: str) -> bool:
        from ..web import format_web_auth_state_diagnosis, inspect_web_auth_state, normalize_web_config
        ...
        if not source_id:
            self.console.print(
                f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Could not resolve a Web source to inspect.[/{self.theme.CORAL_SOFT}]"
            )
            return True

        try:
            auth_state = inspect_web_auth_state(web_cfg, source_id)
        except Exception as exc:
            self.console.print(
                f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Failed to inspect Web auth state: {escape(str(exc))}[/{self.theme.CORAL_SOFT}]"
            )
            return True

        lines = [
            f"[{self.theme.BLUE_SOFT}]Source:[/{self.theme.BLUE_SOFT}] {escape(source_id)}",
            f"[{self.theme.BLUE_SOFT}]Browser profile:[/{self.theme.BLUE_SOFT}] {escape(str(auth_state.get('browser_profile', '') or '(auto)'))}",
            f"[{self.theme.BLUE_SOFT}]Browser account:[/{self.theme.BLUE_SOFT}] {escape(str(auth_state.get('browser_account_email', '') or '(unknown)'))}",
            f"[{self.theme.BLUE_SOFT}]Arena cookie present:[/{self.theme.BLUE_SOFT}] {'YES' if auth_state.get('arena_cookie_present') else 'NO'}",
            f"[{self.theme.BLUE_SOFT}]Arena cookie names:[/{self.theme.BLUE_SOFT}] {escape(', '.join(auth_state.get('arena_cookie_names', []) or []) or '(none)')}",
            f"[{self.theme.BLUE_SOFT}]Arena session email:[/{self.theme.BLUE_SOFT}] {escape(str(auth_state.get('arena_session_email', '') or '(none)'))}",
            f"[{self.theme.BLUE_SOFT}]Google session visible:[/{self.theme.BLUE_SOFT}] {'YES' if auth_state.get('google_web_signed_in') else 'NO'}",
            f"[{self.theme.BLUE_SOFT}]Google session cookies:[/{self.theme.BLUE_SOFT}] {escape(', '.join(auth_state.get('google_session_cookies', []) or []) or '(none)')}",
            f"[{self.theme.BLUE_SOFT}]Source DB arena cookies:[/{self.theme.BLUE_SOFT}] {escape(', '.join(auth_state.get('source_profile_arena_auth_cookie_names', []) or []) or '(none)')}",
            f"[{self.theme.BLUE_SOFT}]Source DB Google cookies:[/{self.theme.BLUE_SOFT}] {escape(', '.join(auth_state.get('source_profile_google_auth_cookie_names', []) or []) or '(none)')}",
            f"[{self.theme.BLUE_SOFT}]Page login visible:[/{self.theme.BLUE_SOFT}] {'YES' if auth_state.get('page_login_visible') else 'NO'}",
            f"[{self.theme.BLUE_SOFT}]Page URL:[/{self.theme.BLUE_SOFT}] {escape(str(auth_state.get('page_url', '') or '(unknown)'))}",
        ]
        diagnosis = format_web_auth_state_diagnosis(source_id, auth_state)
        if diagnosis:
            lines.append(f"[{self.theme.AMBER_GLOW}]Diagnosis:[/{self.theme.AMBER_GLOW}] {escape(diagnosis)}")

        self.console.print()
        self.console.print(
            Panel(
                Text.from_markup("\n".join(lines)),
                title=f"[bold {self.theme.PINK_SOFT}]{self.deco.SPARKLE} Web Auth State {self.deco.SPARKLE}[/bold {self.theme.PINK_SOFT}]",
                border_style=self.theme.BORDER_PRIMARY,
                box=box.ROUNDED,
                padding=(1, 2),
            )
    ...
    def _cmd_web_setting_ui(self) -> bool:
        try:
            import msvcrt
        except ImportError:
            self.console.print(f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Interactive Web settings UI is only supported on Windows.[/{self.theme.CORAL_SOFT}]")
            self.console.print(f"[{self.theme.TEXT_DIM}]Use /web status or the direct /web subcommands instead.[/{self.theme.TEXT_DIM}]")
            return True

        from ..config import default_text_to_image_config, normalize_tti_source
        from ..web import (
        ...
        config = config_manager.load()
        web_cfg = normalize_web_config(getattr(config, "web", {}))
        tti_cfg = dict(config.text_to_image) if isinstance(config.text_to_image, dict) else default_text_to_image_config()
        tti_cfg["active_source"] = normalize_tti_source(tti_cfg.get("active_source", "local"))
        selected_idx = 0
        scroll_offset = 0
        changed = False
        ...
        def build_items() -> List[Dict[str, Any]]:
            return [
                {"name": "Chat Model", "key": "selected_model_id", "kind": "model_text", "description": "Choose the Web chat model.", "command": "/web model"},
                {"name": "Image Model", "key": "web_default_model_id", "kind": "model_image", "description": "Choose the default Web TTI model.", "command": "/web image-model"},
                {"name": "Auto Start", "key": "auto_start", "kind": "bool", "description": "Auto-launch the bundled relay when needed.", "command": "/web settings"},
                {"name": "API URL", "key": "api_url", "kind": "text", "description": "Base URL for the Web relay.", "command": "/web settings"},
                {"name": "Port", "key": "port", "kind": "int", "min": 1, "max": 65535, "step": 1, "description": "Port for the local Web relay.", "command": "/web port <port>"},
                {"name": "Endpoint", "key": "endpoint", "kind": "text", "description": "Chat endpoint path or full URL override.", "command": "/web endpoint <url|/path|clear>"},
                {"name": "Auth Token", "key": "auth_token", "kind": "secret", "description": "Bearer token used by Reverie and the relay.", "command": "/web auth <token>"},
                {"name": "Headless", "key": "headless", "kind": "bool", "description": "Hide the browser window while the relay runs.", "command": "/web settings"},
                {"name": "Humanize Cursor", "key": "humanize_cursor", "kind": "bool", "description": "Enable human-like cursor motion.", "command": "/web settings"},
                {"name": "Fission", "key": "fission", "kind": "bool", "description": "Keep Firefox site isolation enabled.", "command": "/web settings"},
                {"name": "Queue Buffer", "key": "queue_buffer", "kind": "int", "min": 0, "max": 128, "step": 1, "description": "Extra non-stream queue capacity.", "command": "/web settings"},
                {"name": "Image Limit", "key": "image_limit", "kind": "int", "min": 1, "max": 10, "step": 1, "description": "Maximum image count handled per request.", "command": "/web settings"},
                {"name": "Keepalive", "key": "keepalive_mode", "kind": "choice", "choices": ["comment", "content"], "description": "Streaming heartbeat mode.", "command": "/web settings"},
                {"name": "Browser Path", "key": "browser_path", "kind": "text", "description": "Optional browser executable override.", "command": "/web settings"},
                {"name": "Browser Profile", "key": "browser_profile", "kind": "text", "description": "Optional Chromium profile directory hint such as Default or Profile 1.", "command": "/web settings"},
                {"name": "Browser Account", "key": "browser_account_email", "kind": "text", "description": "Preferred browser account email for auto-discovery.", "command": "/web settings"},
                {"name": "Source Root", "key": "source_root", "kind": "readonly", "description": "Discovered WebAI2API reference root.", "command": "/web status"},
                {"name": "Config Path", "key": "config_path", "kind": "readonly", "description": "Resolved reference YAML config path.", "command": "/web status"},
                {"name": "Service", "key": "service_action", "kind": "action", "description": "Start or stop the local WebAI2API relay.", "command": "/web start or /web stop"},
            ]
        ...
        def service_status() -> Dict[str, Any]:
            return get_web_service_status(normalize_web_config(web_cfg), timeout_seconds=1)

        def display_value(item: Dict[str, Any], status: Dict[str, Any]) -> str:
            kind = str(item.get("kind", "")).strip()
            key = str(item.get("key", "")).strip()
            if kind == "model_text":
                selected = resolve_web_selected_model(web_cfg, model_type="text")
                return escape(str((selected or {}).get("display_name", "(choose)")))
            if kind == "model_image":
                selected = resolve_web_image_model(web_cfg, tti_cfg)
                return escape(str((selected or {}).get("display_name", "(choose)")))
            if kind == "secret":
                return escape(mask_secret(web_cfg.get(key, "")))
            if kind == "readonly":
                return escape(str(web_cfg.get(key, "") or "(empty)"))
            if kind == "action":
                return (
                    f"[{self.theme.MINT_SOFT}]Stop service[/{self.theme.MINT_SOFT}]"
                    if status.get("running")
                    else f"[{self.theme.BLUE_SOFT}]Start service[/{self.theme.BLUE_SOFT}]"
                )
        ...
        def render(status: Dict[str, Any]) -> Group:
            items = build_items()
            selected_item = items[selected_idx]
            selected_chat = resolve_web_selected_model(web_cfg, model_type="text")
            selected_image = resolve_web_image_model(web_cfg, tti_cfg)
            ...
            summary.add_column(style=self.theme.TEXT_DIM, width=14)
            summary.add_column(style=f"bold {self.theme.TEXT_PRIMARY}")
            summary.add_row("Service", "Running" if status.get("running") else "Stopped", "Auto Start", "ON" if bool(web_cfg.get("auto_start", True)) else "OFF")
            summary.add_row("Chat Model", str((selected_chat or {}).get("display_name", "(none)")), "Image Model", str((selected_image or {}).get("display_name", "(none)")))
            summary.add_row("API URL", self._truncate_middle(str(web_cfg.get("api_url", "")), 34), "Endpoint", self._truncate_middle(str(web_cfg.get("endpoint", "")), 28))
            summary.add_row("Port", str(web_cfg.get("port", 3000)), "Keepalive", str(web_cfg.get("keepalive_mode", "comment")))
            summary.add_row("Status", "Unsaved changes" if changed else "Live view", "Focus", str(selected_item.get("name", "")))
            summary.add_row("Models", str(status.get("models_count", 0)), "TTI Source", str(tti_cfg.get("active_source", "local")))

            summary_panel = Panel(
                summary,
                title=f"[bold {self.theme.PINK_SOFT}]{self.deco.SPARKLE} Web Settings {self.deco.SPARKLE}[/bold {self.theme.PINK_SOFT}]",
                subtitle=f"[{self.theme.TEXT_DIM}]Bundled WebAI2API source, model routing, and service controls.[/{self.theme.TEXT_DIM}]",
                border_style=self.theme.BORDER_PRIMARY,
                padding=(0, 2),
                box=box.ROUNDED,
            )
            ...
            table.add_column("Kind", style=self.theme.TEXT_DIM, width=12, no_wrap=True)
            table.add_column("Value", style=self.theme.TEXT_PRIMARY, ratio=1, no_wrap=True)
            end_idx = min(scroll_offset + self._setting_visible_count(), len(items))
            visible_items = items[scroll_offset:end_idx]
            for row_index, item in enumerate(visible_items):
                actual_idx = scroll_offset + row_index
                value_text = display_value(item, status)
                if actual_idx == selected_idx:
                    table.add_row(
                        ...
                        f"[{self.theme.TEXT_PRIMARY}]{escape(str(item.get('kind', '')))}[/{self.theme.TEXT_PRIMARY}]",
                        f"[reverse]{value_text}[/reverse]",
                    )
                else:
                    table.add_row(
            ...
            list_panel = Panel(
                table,
                title=f"[bold {self.theme.BLUE_SOFT}]{self.deco.DIAMOND} Web Source[/bold {self.theme.BLUE_SOFT}]",
                subtitle=f"[{self.theme.TEXT_DIM}]Visible window: {scroll_offset + 1}-{end_idx} / {len(items)}[/{self.theme.TEXT_DIM}]",
                border_style=self.theme.BORDER_SUBTLE,
                padding=(0, 1),
                box=box.ROUNDED,
            )
            ...
            detail_lines = [
                f"[bold {self.theme.PURPLE_SOFT}]{escape(selected_item['name'])}[/bold {self.theme.PURPLE_SOFT}]",
                f"[{self.theme.TEXT_SECONDARY}]{escape(str(selected_item.get('description', '') or ''))}[/{self.theme.TEXT_SECONDARY}]",
                "",
                f"[{self.theme.TEXT_DIM}]Current[/{self.theme.TEXT_DIM}] {display_value(selected_item, status)}",
                "",
                f"[{self.theme.TEXT_DIM}]Quick adjust[/{self.theme.TEXT_DIM}] Use h/l or left/right for bool, int, and choice items.",
                f"[{self.theme.TEXT_DIM}]Precise edit[/{self.theme.TEXT_DIM}] Press Enter for selectors, prompt edits, or service actions.",
                f"[{self.theme.TEXT_DIM}]Command equivalent[/{self.theme.TEXT_DIM}] {escape(str(selected_item.get('command', '') or ''))}",
            ]
            detail_panel = Panel(
                Text.from_markup("\n".join(detail_lines)),
                title=f"[bold {self.theme.PURPLE_SOFT}]{self.deco.DIAMOND} Details[/bold {self.theme.PURPLE_SOFT}]",
            ...
            footer_grid.add_row(
                ...
                Text(
                    f"{selected_idx + 1}/{max(1, len(items))} · {'pending save' if changed else 'ready'}",
                    style=self.theme.AMBER_GLOW if changed else self.theme.TEXT_DIM,
                ),
            )
            footer_panel = Panel(
        ...
        def step_item(item: Dict[str, Any], direction: int) -> bool:
            kind = str(item.get("kind", "")).strip()
            key = str(item.get("key", "")).strip()
            if kind in ("readonly", "secret", "action", "model_text", "model_image", "text"):
                return False
            if kind == "bool":
                web_cfg[key] = not bool(web_cfg.get(key))
                return True
            if kind == "int":
                step = int(item.get("step", 1))
                min_value = int(item.get("min", 0))
                max_value = int(item.get("max", 999999))
                current = int(web_cfg.get(key, min_value))
                current = max(min_value, min(max_value, current + step * direction))
                web_cfg[key] = current
                if key == "port":
                    web_cfg["api_url"] = f"http://127.0.0.1:{current}"
                return True
            if kind == "choice":
                choices = list(item.get("choices", []) or [])
                if not choices:
                    return False
                current = str(web_cfg.get(key, choices[0]))
                try:
                    current_index = choices.index(current)
                except ValueError:
                    current_index = 0
                web_cfg[key] = choices[(current_index + direction) % len(choices)]
                return True
            return False

        def edit_item(item: Dict[str, Any]) -> bool:
            kind = str(item.get("kind", "")).strip()
            key = str(item.get("key", "")).strip()
            if kind == "readonly":
                return False
            if kind == "model_text":
                ...
                selected = self._resolve_catalog_selection(
                    catalog=catalog,
                    current_selected_id=str(web_cfg.get("selected_model_id", "")),
                    model_query="",
                    provider_label="Web",
                )
                if not selected:
            ...
            if kind == "model_image":
                ...
                if not selected:
                    return False
                tti_cfg["web_default_model_id"] = selected["id"]
                tti_cfg["web_default_model_display_name"] = selected["display_name"]
                return True
            if kind == "bool":
                web_cfg[key] = not bool(web_cfg.get(key))
                return True
            if kind == "int":
                raw = Prompt.ask(item["name"], default=str(web_cfg.get(key, item.get("min", 0)))).strip()
                try:
                    parsed = int(raw)
                except ValueError:
                    return False
                if parsed < int(item.get("min", 0)) or parsed > int(item.get("max", 999999)):
                    return False
                web_cfg[key] = parsed
                if key == "port":
                    web_cfg["api_url"] = f"http://127.0.0.1:{parsed}"
                return True
            if kind == "choice":
                picked = Prompt.ask(item["name"], default=str(web_cfg.get(key, "")), choices=[str(x) for x in item.get("choices", [])]).strip()
                web_cfg[key] = picked
                return True
            if kind in ("text", "secret"):
                prompt_kwargs: Dict[str, Any] = {"default": str(web_cfg.get(key, "") or "")}
                if kind == "secret":
                    prompt_kwargs["password"] = True
                picked = Prompt.ask(item["name"], **prompt_kwargs).strip()
                web_cfg[key] = picked
                return True
            if kind == "action":
                ...
                if status.get("running"):
                    result = stop_web_service(web_cfg)
                    if not result.get("success"):
                        self.console.print(f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} {escape(str(result.get('error', 'Failed to stop WebAI2API')))}[/{self.theme.CORAL_SOFT}]")
                else:
                    result = start_web_service(web_cfg)
                    if not result.get("success"):
                        self.console.print(f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} {escape(str(result.get('error', 'Failed to start WebAI2API')))}[/{self.theme.CORAL_SOFT}]")
                return False
            return False
            ...
            while True:
                if msvcrt.kbhit():
                    ...
                    if key in (b"k", b"K"):
                        ...
                        changed = step_item(selected_item, -1) or changed
                    elif key in (b"l", b"L", b"d", b"D", b" "):
                        changed = step_item(selected_item, 1) or changed
                    elif key == b"\r":
                        live.stop()
                        changed = edit_item(selected_item) or changed
                        live.start()
                    elif key in (b"\x00", b"\xe0"):
                        key = msvcrt.getch()
                        if key == b"H":
                    ...
                    live.update(render(service_status()), refresh=True)
                current_size = self._console_size()
                if current_size != last_size:
                    last_size = current_size
                    items = build_items()
        ...
        if changed:
            config.web = normalize_web_config(web_cfg)
            config.text_to_image = tti_cfg
            config_manager.save(config)
        ...
        self.console.print()
        self.console.print(
            f"[{self.theme.MINT_VIBRANT if changed else self.theme.TEXT_DIM}]"
            f"{self.deco.CHECK_FANCY if changed else self.deco.DOT_MEDIUM} "
            f"{'Web settings saved.' if changed else 'Web settings closed without changes.'}"
            f"[/{self.theme.MINT_VIBRANT if changed else self.theme.TEXT_DIM}]"
        )
        self.console.print()
        return True
