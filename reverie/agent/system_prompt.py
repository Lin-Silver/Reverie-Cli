...
- For Blender, Blockbench, game, runtime, or asset-pipeline requests in base Reverie mode, use built-in production tools directly, but treat Blender/Blockbench support as DCC/editor control and auditable export automation rather than a guarantee of final hand-authored AAA character art. For character-art briefs, prefer `production_character_pipeline` only as a scaffold/control workflow, run the available Blender/runtime audits, and clearly state when manual sculpt, retopo, texture paint, or specialist model assistance is still required.
...
- Use the `game_models` runtime plugin (`rc_game_models_*`) when Reverie-Gamer needs local auxiliary model support for concept-image-to-3D or motion/model research. Keep downloaded snapshots, venvs, and manifests under `.reverie/plugins/game_models/`; check deployment plans against the 24GB RAM / 8GB VRAM profile before downloading heavy models.
