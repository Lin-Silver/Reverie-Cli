...
impl EventHandler {
    ...
    /// 处理 crossterm 事件
    pub fn poll(&self) -> Result<bool, Box<dyn std::error::Error>> {
...
pub mod keys {
    ...
    /// 复制 (Ctrl+Shift+C)
    pub const COPY: (KeyCode, KeyModifiers) = (KeyCode::Char('c'), KeyModifiers::CONTROL | KeyModifiers::SHIFT);
    ...
    /// 检查是否是快捷键
    pub fn is_key(key_code: KeyCode, modifiers: KeyModifiers, key: (KeyCode, KeyModifiers)) -> bool {
