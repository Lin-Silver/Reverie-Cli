...
class RatsCatalogTool(BaseTool):
    ...
    always_load = True
    # `description` is a property below: the introduction has to name whichever
    # services are connected right now, which is not knowable at class scope.
    parameters = {
        "type": "object",
        "properties": {
            "operation": {
    ...
    def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            if operation == "list":
                ...
                # runtime's 16-name request cap. Either way, saying nothing would
                # leave the model believing it asked and received.
