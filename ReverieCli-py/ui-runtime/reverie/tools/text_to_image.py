...
- Generate with Agnes: {"action": "generate", "source": "agnes", "model": "agnes-image-2.1-flash", "prompt": "a vase of flowers"}"""
    ...
    def _resolve_script_path(self, config: Dict[str, Any], script_override: Optional[str] = None) -> Path:
        ...
        candidates.append((self._project_root.parent / "comfy" / "generate_image.py").resolve())
        candidates.append((self._project_root / "Comfy" / "generate_image.py").resolve())
    ...
    def _get_bundled_comfy_dir(self) -> Optional[Path]:
        ...
        shared_bundled = self._project_root.parent / "comfy"
        if (shared_bundled / "generate_image.py").exists():
            return shared_bundled
