...
HELP_TOPICS: Dict[str, Dict[str, object]] = {
    ...
    "mcp": {
        ...
        "detail": "Bare `/mcp` opens an interactive panel similar to `/setting`, with live MCP state, quick toggles, discovery refresh, and server add/remove flows. Reverie stores MCP settings beside the executable in `.\\.Reverie\\MCP.json`, discovers tools from configured servers, and exposes them to the model as dynamic `mcp_<server>_<tool>` functions. `url` follows the legacy SSE transport and `httpUrl` follows streamable HTTP, matching Codex/Gemini/Qwen MCP conventions. Reverie-Gamer now seeds a built-in `ashfox` MCP server entry automatically.",
