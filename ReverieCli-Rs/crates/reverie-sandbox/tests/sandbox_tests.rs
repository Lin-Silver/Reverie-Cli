...
mod tests {
    use super::*;
    use crate::policy::{SandboxPolicy, FileRule, FileAccessMode, NetworkRule, ProcessLimits, ResourceLimits};
    ...
    fn test_create_sandbox_with_custom_id() {
        let mut manager = SandboxManager::new();
        let id = manager.create_sandbox(Some("my-sandbox".to_string()), None).unwrap();
    ...
    fn test_sandbox_file_access_allowed() {
        ...
        policy.file_rules = vec![
            FileRule::allow_read_write("/tmp"),
            FileRule::deny_all(),
        ];
        
        let mut sandbox = crate::manager::SandboxInstance::new(
            "test".to_string(),
            policy,
        );
        
        let path = std::path::Path::new("/tmp/test.txt");
        assert!(sandbox.check_file_access(path, FileAccessMode::Read).is_ok());
        assert!(sandbox.check_file_access(path, FileAccessMode::ReadWrite).is_ok());
    }
    ...
    fn test_sandbox_file_access_denied() {
        ...
        let path = std::path::Path::new("/home/user/file.txt");
        assert!(sandbox.check_file_access(path, FileAccessMode::Read).is_err());
    }
    ...
    fn test_sandbox_network_access_allowed() {
        ...
        assert!(sandbox.check_network_access("localhost", Some(8080)).is_ok());
    }
    ...
    fn test_sandbox_network_access_denied() {
        ...
        assert!(sandbox.check_network_access("example.com", Some(80)).is_err());
    }
