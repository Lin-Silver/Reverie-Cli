...
HELP_TOPICS: Dict[str, Dict[str, object]] = {
    ...
    "plugins": {
        ...
        "detail": "Plugins are now positioned as Reverie's local SDK depot rather than another Skill/MCP-like prompt feature. `.reverie/plugins/<plugin-id>/runtime` is where heavyweight portable runtimes such as Blender, Godot, Blockbench, or validators can live beside the executable. Plugins may still expose optional `rc_*` dynamic tools when a runnable entry supports the Reverie CLI `-RC` protocol, and `/plugins templates` remains available for authoring those wrappers.",
