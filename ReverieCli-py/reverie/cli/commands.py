...
class CommandHandler:
    ...
    def cmd_total(self, args: str) -> bool:
        ...
        from ..agent_regression import latest_agent_regression_summary, run_agent_regression
        from ..lifecycle import summarize_lifecycle
        ...
        requested_action = args.strip().lower()
        if requested_action in {"benchmark", "bench", "regression", "regressions", "run"}:
            project_data_dir = self.app.get("project_data_dir")
            project_root = self.app.get("project_root")
            if not project_data_dir:
                self.console.print(f"[{self.theme.CORAL_SOFT}]{self.deco.CROSS} Agent regression cache is not available in this session.[/{self.theme.CORAL_SOFT}]")
                return True
            summary = run_agent_regression(Path(project_data_dir), project_root=Path(project_root) if project_root else None)
            self._render_agent_regression_summary(summary)
            return True
        ...
        lifecycle_summary = summarize_lifecycle(
            cache_dir,
            project_root=Path(str(selected_workspace.get("workspace_path") or "")) if selected_workspace.get("workspace_path") else None,
        )
        regression_summary = latest_agent_regression_summary(
            cache_dir,
            project_root=Path(str(selected_workspace.get("workspace_path") or "")) if selected_workspace.get("workspace_path") else None,
        )
        ...
        runtime_table = Table(
            title=f"[bold {self.theme.AMBER_GLOW}]{self.deco.CRYSTAL} Agent Lifecycle[/bold {self.theme.AMBER_GLOW}]",
            box=box.ROUNDED,
            border_style=self.theme.BORDER_SECONDARY,
            expand=True,
        )
        runtime_table.add_column("Surface", style=f"bold {self.theme.AMBER_GLOW}", no_wrap=True)
        runtime_table.add_column("Status", style=self.theme.TEXT_SECONDARY)
        runtime_table.add_column("Signal", style=self.theme.TEXT_DIM)
        runtime_table.add_row(
            "Hooks",
            f"{int(lifecycle_summary.get('events', 0) or 0):,} events",
            f"{int(lifecycle_summary.get('denied', 0) or 0):,} denied, {len(lifecycle_summary.get('rules', []) or []):,} rules",
        )
        regression_state = "not run" if regression_summary.get("passed") is None else ("passing" if regression_summary.get("passed") else "failing")
        runtime_table.add_row(
            "Regression",
            regression_state,
            f"{int(regression_summary.get('passed_count', 0) or 0)}/{int(regression_summary.get('total', 0) or 0)} scenarios, score {int(regression_summary.get('score', 0) or 0)}",
        )
        runtime_table.add_row(
            "Audit Store",
            str(Path(str(lifecycle_summary.get("audit_path") or cache_dir / "lifecycle" / "audit.jsonl")).name),
            str(lifecycle_summary.get("audit_path") or ""),
        )
        self.console.print(runtime_table)

        recent_audit = list(lifecycle_summary.get("recent") or [])[-8:]
        audit_table = Table(
            title=f"[bold {self.theme.MINT_SOFT}]{self.deco.CRYSTAL} Recent Hook Audit[/bold {self.theme.MINT_SOFT}]",
            box=box.ROUNDED,
            border_style=self.theme.BORDER_SECONDARY,
            expand=True,
        )
        audit_table.add_column("#", style=self.theme.TEXT_DIM, width=4, justify="right")
        audit_table.add_column("Time", style=self.theme.TEXT_DIM, no_wrap=True)
        audit_table.add_column("Phase", style=self.theme.TEXT_SECONDARY, no_wrap=True)
        audit_table.add_column("Tool", style=f"bold {self.theme.MINT_SOFT}", no_wrap=True)
        audit_table.add_column("Outcome", style=self.theme.TEXT_SECONDARY)
        for index, row in enumerate(recent_audit, start=1):
            outcome = "denied" if row.get("allowed") is False else ("ok" if row.get("success") is not False else "failed")
            audit_table.add_row(
                str(index),
                str(row.get("timestamp") or "")[:19].replace("T", " "),
                str(row.get("phase") or ""),
                str(row.get("tool") or ""),
                f"{outcome} {str(row.get('reason') or '')}".strip(),
            )
        if not recent_audit:
            audit_table.add_row("", "No hook audit events yet", "", "", "")
        ...
        try:
            choice = Prompt.ask(
                f"[{self.theme.PURPLE_SOFT}]Open session #, [a]udit detail, [h]ooks, or [b]enchmark[/{self.theme.PURPLE_SOFT}]",
                default="",
            ).strip()
        except KeyboardInterrupt:
            self.console.print()
        ...
        if lowered_choice in {"a", "audit"}:
            self.console.print_json(data=lifecycle_summary.get("recent") or [])
            return True
        if lowered_choice in {"h", "hook", "hooks"}:
            self.console.print_json(data=lifecycle_summary.get("rules") or [])
            return True
        if lowered_choice in {"b", "bench", "benchmark", "regression"}:
            run_summary = run_agent_regression(
                cache_dir,
                project_root=Path(str(selected_workspace.get("workspace_path") or "")) if selected_workspace.get("workspace_path") else None,
            )
            self._render_agent_regression_summary(run_summary)
            return True

        if not session_rows:
            self.console.print(f"[{self.theme.AMBER_GLOW}]{self.deco.DOT_MEDIUM} No sessions are available for transcript inspection.[/{self.theme.AMBER_GLOW}]")
            return True

        if not choice.isdigit():
            self.console.print(f"[{self.theme.AMBER_GLOW}]{self.deco.DOT_MEDIUM} Enter a session number, a, h, or b.[/{self.theme.AMBER_GLOW}]")
            return True
    ...
    def _render_agent_regression_summary(self, summary: Dict[str, Any]) -> None:
        """Render deterministic agent behavior regression results."""
        table = Table(
            title=f"[bold {self.theme.AMBER_GLOW}]{self.deco.CRYSTAL} Agent Behavior Regression[/bold {self.theme.AMBER_GLOW}]",
            box=box.ROUNDED,
            border_style=self.theme.BORDER_SECONDARY,
            expand=True,
        )
        table.add_column("Scenario", style=f"bold {self.theme.BLUE_SOFT}")
        table.add_column("Result", style=self.theme.TEXT_SECONDARY, no_wrap=True)
        table.add_column("Time", style=self.theme.TEXT_DIM, justify="right", no_wrap=True)
        table.add_column("Detail", style=self.theme.TEXT_DIM)
        for row in summary.get("results", []) or []:
            passed = bool(row.get("passed"))
            table.add_row(
                str(row.get("title") or row.get("id") or ""),
                f"[{self.theme.MINT_SOFT}]pass[/{self.theme.MINT_SOFT}]" if passed else f"[{self.theme.CORAL_SOFT}]fail[/{self.theme.CORAL_SOFT}]",
                f"{int(row.get('duration_ms', 0) or 0)}ms",
                str(row.get("detail") or ""),
            )
        if not (summary.get("results") or []):
            table.add_row("No regression run recorded", "", "", "")
        self.console.print(table)
        state = "passing" if summary.get("passed") else "failing"
        self.console.print(
            f"[{self.theme.TEXT_SECONDARY}]Agent regression is {state}: "
            f"{int(summary.get('passed_count', 0) or 0)}/{int(summary.get('total', 0) or 0)} scenarios, "
            f"score {int(summary.get('score', 0) or 0)}. "
            f"Summary: {escape(str(summary.get('summary_path') or ''))}[/{self.theme.TEXT_SECONDARY}]"
        )
