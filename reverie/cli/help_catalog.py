...
HELP_TOPICS: Dict[str, Dict[str, object]] = {
    ...
    "mcp": {
        ...
        "detail": "Bare `/mcp` opens an interactive panel similar to `/setting`, with live MCP state, quick toggles, discovery refresh, and server add/remove flows. Reverie stores MCP settings beside the executable in `.\\.Reverie\\MCP.json`, discovers tools from configured servers, and exposes them to the model as dynamic `mcp_<server>_<tool>` functions. `url` follows the legacy SSE transport and `httpUrl` follows streamable HTTP, matching Codex/Gemini/Qwen MCP conventions.",
        ...
        "subcommands": [
            ...
            {"usage": "/mcp reload", "description": "Reload `.Reverie/MCP.json` and refresh the discovered MCP catalog."},
    ...
    "engine": {
        ...
        "overview": "profile, create, sample, run, validate, smoke, health, benchmark, package, test",
        "subcommands": [
        ...
        "examples": ["/engine", "/engine create", "/engine sample iso_adventure", "/engine validate", "/engine health", "/engine benchmark", "/engine package", "/engine smoke", "/engine test"],
    },
    "playtest": {
        "command": "/playtest",
        "section": "Game",
