...
def build_atlas_additional_rules(
    ...
    *,
    ...
    lines: List[str] = [
        ...
        "- Treat `artifacts/` as the Atlas document system of record; on a fresh or resumed session, inspect those artifacts before relying on conversational memory.",
        "- Keep `artifacts/task.md` synchronized as a detailed task tree that mirrors the Atlas delivery slices and marks completed items with `[x]`.",
        "- Keep `artifacts/atlas/resume_index.md` synchronized as the first artifact a resumed Atlas session should inspect.",
        "",
