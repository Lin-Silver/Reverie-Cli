...
use crate::policy::{SandboxPolicy, SandboxError, SandboxResult, AuditEvent, AuditEventType, AuditEventDetails, AuditResult};
...
impl SandboxInstance {
    ...
    /// 记录审计事件
    fn log_audit(&mut self, event_type: AuditEventType, details: AuditEventDetails, result: AuditResult) {
    ...
    /// 检查文件访问权限
    pub fn check_file_access(&mut self, path: &std::path::Path, mode: crate::policy::FileAccessMode) -> SandboxResult<()> {
            ...
            if let Some(rule_mode) = rule.matches(path) {
                ...
                match (rule_mode, mode) {
                    (crate::policy::FileAccessMode::Deny, _) => {
                        ...
                        return Err(SandboxError::PermissionDenied(format!("Access denied to: {}", path_str)));
                    }
                    (crate::policy::FileAccessMode::Read, crate::policy::FileAccessMode::ReadWrite)
                    | (crate::policy::FileAccessMode::Read, crate::policy::FileAccessMode::Write) => {
                        ...
                        return Err(SandboxError::PermissionDenied(format!("Write access denied to: {}", path_str)));
                    }
                    _ => {
                        self.log_audit(
                            AuditEventType::FileAccess,
        ...
        Err(SandboxError::PermissionDenied(format!("No rule for: {}", path_str)))
    }
    ...
    pub fn check_network_access(&mut self, host: &str, port: Option<u16>) -> SandboxResult<()> {
        for rule in &self.policy.network_rules {
            if rule.matches(host) {
                if rule.allowed {
                    ...
                    return Err(SandboxError::NetworkAccessDenied(format!("Access denied to: {}", host)));
                }
            }
        }
        ...
        Err(SandboxError::NetworkAccessDenied(format!("No rule for: {}", host)))
    }
    ...
    pub fn check_command(&self, command: &str) -> SandboxResult<()> {
        ...
        if self.policy.process_limits.denied_commands.contains(command) {
            return Err(SandboxError::PermissionDenied(format!("Command '{}' is denied", command)));
        }
        
        // 如果没有允许的命令列表，或者命令在允许列表中
        if self.policy.process_limits.allowed_commands.is_empty()
            || self.policy.process_limits.allowed_commands.contains(command)
        {
            return Ok(());
        }
        
        Err(SandboxError::PermissionDenied(format!("Command '{}' is not allowed", command)))
    }
...
impl SandboxManager {
    ...
    /// 创建新的沙箱实例
    pub fn create_sandbox(&mut self, id: Option<String>, policy: Option<SandboxPolicy>) -> SandboxResult<String> {
