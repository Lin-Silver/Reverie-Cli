...
- 流式输出和工具执行链路做了重构，简单任务的响应速度、SSE 解析稳定性和 Windows 命令执行兼容性都做了明显优化。
- TUI 现在支持运行中工具实时输出面板、`Ctrl+T` 任务抽屉、更紧凑的工具结果卡片，以及更干净的流式正文排版。
- 设置系统新增 `/settings`、`/setting tool-output ...`、`/setting thinking ...`，旧 `config.json` 也会自动补齐新的显示偏好字段。

完整版本说明见 [更新日志](changelog.md)。

## 游戏创建功能更新预期

截至 **2026-04-04**，`Reverie-Gamer` 的目标仍然是：

`一句提示词 -> 蓝图 -> 脚手架 -> 首个可玩切片 -> 持续扩展`

当前预估节奏：

- **2026-04-11 到 2026-04-18**：下一轮可玩切片稳定性更新，重点是 prompt 到项目骨架再到首个可运行切片的闭环
- **2026 年 4 月下旬**：补强资产流水线与验证闭环集成
- **2026 年 5 月**：继续推进长周期项目续作能力和更丰富的多区域生成

更完整的 Gamer 评估与路线图见 [Reverie-Gamer 大型 3D 游戏升级路线图](reverie_gamer_3d_game_generation_assessment.md)。
