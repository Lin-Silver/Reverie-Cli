...
def test_default_runtime_catalog_excludes_engines_integrated_into_core() -> None:
    catalog_ids = {item.plugin_id for item in DEFAULT_RUNTIME_PLUGIN_CATALOG}

    assert "blender" in catalog_ids
    assert "godot" not in catalog_ids
    assert "o3de" not in catalog_ids
    assert "blockbench" not in catalog_ids
    assert "unity" not in catalog_ids
    assert "unreal" not in catalog_ids


def test_integrated_o3de_is_not_materialized_as_plugin_sdk(tmp_path: Path) -> None:
    app_root = tmp_path / "app"
    manager = RuntimePluginManager(app_root)
    ...
    assert "Unknown SDK plugin id" in status["error"]


def test_integrated_godot_is_not_materialized_as_plugin_sdk(tmp_path: Path) -> None:
    app_root = tmp_path / "app"
    manager = RuntimePluginManager(app_root)
