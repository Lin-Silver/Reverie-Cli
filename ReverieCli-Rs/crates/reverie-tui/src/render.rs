...
use crate::components::{
    MessageDisplay, InputBox, ToolCallPanel, SessionList, HelpPanel, StatusBar, MainLayout,
};
...
impl TuiRenderer {
    ...
    pub fn render(&mut self, app_state: &AppState) -> Result<(), Box<dyn std::error::Error>> {
        self.terminal.draw(|frame| {
            ...
            // 主布局
            let (message_area, input_area, sidebar_area) = self.layout.split(area);
            
            // 渲染侧边栏（会话列表）
            if self.layout.sidebar_ratio > 0 {
    ...
    /// 渲染消息区域
    fn render_messages(&self, frame: &mut Frame, area: Rect, app_state: &AppState) {
        ...
        if app_state.is_searching && !app_state.search_query.is_empty() {
            self.render_search_highlight(frame, inner_area, visible_messages, &app_state.search_query);
        }
    }
    ...
    fn render_search_highlight(
        ...
        let paragraph = Paragraph::new(search_status)
    ...
    /// 渲染输入区域
    fn render_input(&self, frame: &mut Frame, area: Rect, app_state: &AppState) {
    ...
    /// 渲染侧边栏
    fn render_sidebar(&self, frame: &mut Frame, area: Rect, app_state: &AppState) {
        let selected_index = app_state.sessions.iter().position(|s| {
            app_state.current_session.as_ref().map(|cs| cs.id == s.id).unwrap_or(false)
        });
    ...
    /// 渲染帮助面板
    fn render_help_overlay(&self, frame: &mut Frame, area: Rect, theme: &crate::state::Theme) {
        // 创建半透明背景
        let block = Block::default()
        ...
        let items: Vec<ListItem> = sessions
            ...
            .map(|session| {
                let timestamp = chrono::DateTime::from_timestamp(
                    session.created_at as i64 / 1000,
                    0
                )
