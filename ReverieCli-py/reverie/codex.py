...
_CODEX_LOCAL_SOURCE_MODELS_RELATIVE_PATHS = (
    ...
    ("references", "agent_comparison", "codex", "codex-rs", "models-manager", "models.json"),
    ("references", "codex-main", "codex-rs", "models-manager", "models.json"),
    ("references", "codex-main", "codex-rs", "core", "models.json"),
)
...
def _iter_local_codex_source_model_paths() -> List[Path]:
    ...
    def add_candidate(root: Optional[Path], relative_path: Tuple[str, ...]) -> None:
        if root is None:
            return
        try:
            ...
            return
        candidate = resolved_root.joinpath(*relative_path)
    ...
    def iter_roots(root: Optional[Path]) -> List[Path]:
        if root is None:
            return []
        try:
    ...
    roots = (
        ...
        cwd.parent,
        argv_root,
        argv_root.parent if argv_root else None,
        executable_root,
        executable_root.parent if executable_root else None,
    )
    for root in roots:
        for candidate_root in iter_roots(root):
            for relative_path in _CODEX_LOCAL_SOURCE_MODELS_RELATIVE_PATHS:
                add_candidate(candidate_root, relative_path)
    return candidates
...
def _merge_source_catalog_with_allowed_models(source_catalog: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    if not source_catalog:
        ...
        fallback.sort(
            key=lambda item: (
                order.get(str(item.get("id", "")).strip().lower(), len(order)),
                _CODEX_ALLOWED_MODEL_IDS.index(item["id"]) if item.get("id") in _CODEX_ALLOWED_MODEL_IDS else 999,
            )
        )
        return fallback
    return [dict(item) for item in source_catalog]
...
def _codex_content_parts(content: Any, *, role: str) -> List[Dict[str, Any]]:
    """Translate OpenAI chat-style multimodal content into Responses input parts."""
    ...
    for part in content:
        ...
        if part_type in {"text", "input_text", "output_text"}:
            ...
            continue
        if part_type != "image_url" or role == "assistant":
            continue
...
def build_codex_request_payload(
    ...
    for message in messages:
        ...
        item["content"].extend(_codex_content_parts(message.get("content", ""), role=role))
