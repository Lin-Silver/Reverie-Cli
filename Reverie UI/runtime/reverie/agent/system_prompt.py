...
- Use the `game_models` runtime plugin (`rc_game_models_*`) when Reverie-Gamer needs local auxiliary model support for text-to-3D, concept-image-to-3D, or motion/model research. Keep downloaded snapshots, caches, venvs, and manifests under `.reverie/plugins/game_models/`; for 24GB RAM / 8GB VRAM, TRELLIS Text XLarge is selectable with `profile=low_vram`, while HY-Motion remains an explicit guarded download.
