...
class ReverieUiBridge:
    ...
    def dispatch(self, message: Dict[str, Any]) -> None:
        ...
        handlers: Dict[str, Callable[[Any, Dict[str, Any]], None]] = {
            ...
            "getTotals": self.handle_get_totals,
            "runAgentRegression": self.handle_run_agent_regression,
    ...
    def summarize_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "active_model": object_to_dict(
                active_model,
                ("model", "model_display_name", "base_url", "provider", "supports_vision", "endpoint", "thinking_mode", "max_context_tokens"),
            )
            if active_model
            else None,
            "models": models,
        ...
        sources.append(
            {
                ...
                "reasoning_choices": get_codex_reasoning_catalog(
                    str(codex_cfg.get("selected_model_id", "") or ""),
        ...
        sources.append(
            {
                ...
                "thinking_options": get_nvidia_thinking_options(selected_nvidia_id),
    ...
    def infer_plugin_manifest_from_assets(self, release: Dict[str, Any], assets: list[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "rust_cli_asset": next((asset for asset in assets if asset["name"].lower() == "reverie-rust-preview.exe"), None),
            "plugins": plugins,
            "inferred": True,
        }
    ...
    def summarize_totals(self) -> Dict[str, Any]:
        from reverie.agent_regression import latest_agent_regression_summary

        interface = self.ensure_interface()
        try:
            interface.workspace_stats_manager.flush()
        except Exception:
            pass

        dashboard = interface.workspace_stats_manager.build_dashboard_data()
        lifecycle_manager = getattr(interface, "lifecycle_manager", None)
        lifecycle = lifecycle_manager.summary() if lifecycle_manager else {"enabled": False, "events": 0, "recent": [], "rules": []}
        regression = latest_agent_regression_summary(interface.project_data_dir, project_root=interface.project_root)
        return {
            "workspace": {
                "name": dashboard.get("workspace_name"),
                "path": dashboard.get("workspace_path") or str(interface.project_root),
                "cache_dir": str(interface.project_data_dir),
            },
            "usage": dashboard,
            "lifecycle": lifecycle,
    ...
    def handle_get_totals(self, request_id: Any, payload: Dict[str, Any]) -> None:
        self.ensure_interface()
        emit({"id": request_id, "type": "totals", "totals": self.summarize_totals()})

    def handle_run_agent_regression(self, request_id: Any, payload: Dict[str, Any]) -> None:
        from reverie.agent_regression import run_agent_regression

        interface = self.ensure_interface()
        emit({"id": request_id, "type": "agent.regression.started"})
        summary = run_agent_regression(interface.project_data_dir, project_root=interface.project_root)
        emit({"id": request_id, "type": "agent.regression.complete", "summary": summary})
        emit({"id": request_id, "type": "totals", "totals": self.summarize_totals()})
    ...
    def handle_save_model(self, request_id: Any, payload: Dict[str, Any]) -> None:
        ...
        model = ModelConfig(
            ...
            supports_vision=bool(payload.get("supports_vision", payload.get("vision", False))),
